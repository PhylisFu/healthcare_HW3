...
str(revenue)
#select Unique number, hospital number, admission type, admission source, age, sex, discharge status, primary payer, charges, diagonosis, procedure and patient days columns in inpatient
...
```{r}
#patient 859382
a<-inpatient_q1 %>%  filter(UNIQ ==859382)

b<-ED_q1 %>% filter(UNIQ ==859382)
...
```{r}
drug_abuse %>% group_by(sex) %>% summarise(n=n())
```
...
list<-data %>% gather(key,value) %>% group_by(value) %>% tally  %>% filter(str_sub(value, 1, 3) %in% code) %>% arrange(desc(n)) %>% top_n(10)
```
